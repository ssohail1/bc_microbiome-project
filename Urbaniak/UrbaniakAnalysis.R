...
treeNJudat <- NJ(dmudat) # Note, tip order != sequence order

## Internal maximum likelihood
fitudat = pml(treeNJudat, data=phang.alignudat)
### negative edges length changed to 0!

## negative edges length changed to 0!
fitGTRudat <- update(fitudat, k=4, inv=0.2)
fitGTRudat <- optim.pml(fitGTRudat, model="GTR", optInv=TRUE, optGamma=TRUE,
                    rearrangement = "stochastic", control = pml.control(trace = 0))

# Import into phyloseq
mapudat <- import_qiime_sample_data("/media/mbb/Sidras_Projects/Urbaniak_paper/fastqfiles/UrbaniakMetadata.txt")

## Assigning species
taxa.plusudat <- addSpecies(taxasilvaudat, "/media/mbb/Sidras_Projects/testfolderurbaniak/silva_species_assignment_v138.1.fa", verbose=TRUE)
...
colnames(taxa.plusudat) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus","Species")
unname(taxa.plusudat)

## Making phyloseq object
psudatphy <- phyloseq(otu_table(seqtab.nochimudat, taxa_are_rows=FALSE), 
               tax_table(taxa.plusudat),phy_tree(fitGTRudat$tree))

## Merge PhyloSeq object with map
psudatphy <- merge_phyloseq(psudatphy,mapudat)
psudatphy
...
set.seed(711)
phy_tree(psudatphy) <- root(phy_tree(psudatphy), sample(taxa_names(psudatphy), 1), resolve.root = TRUE)
is.rooted(phy_tree(psudatphy))
## TRUE

urbaniaktree <- phy_tree(psudatphy)
...
## Unweighted UniFrac
UniFrac(psudatphy) #Weighted default is False
unweightedudat <- UniFrac(psudatphy)
## this works - makes a scatterplot
plot.default(unweightedudat)
...
## Weighted UniFrac
UniFrac(psudatphy, weighted = TRUE)
weightedudat <- UniFrac(psudatphy, weighted = TRUE)
## this works - makes a scatterplot
plot.default(weightedudat)
